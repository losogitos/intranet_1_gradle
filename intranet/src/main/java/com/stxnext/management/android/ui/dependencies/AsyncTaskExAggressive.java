...
public abstract class AsyncTaskExAggressive<Params, Progress, Result> {
    ...
    private static final ThreadFactory aggressiveThreadFactory = new ThreadFactory() {
        private final AtomicInteger mCount = new AtomicInteger(1);

        public Thread newThread(Runnable r) {
            return new Thread(r, "AsyncTaskExAggressive #" + mCount.getAndIncrement());
        }
    };

    private static final ThreadPoolExecutor aggressiveExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE,
            MAXIMUM_POOL_SIZE, KEEP_ALIVE, TimeUnit.SECONDS, aggressiveWorkQueue, aggressiveThreadFactory);
    ...
    public AsyncTaskExAggressive(final int priority) {
            ...
            protected void done() {
                ...
                try {
                    ...
                    message = sHandler.obtainMessage(MESSAGE_POST_CANCEL,
                            new AsyncTaskExResult<Result>(AsyncTaskExAggressive.this, (Result[]) null));
                ...
                message = sHandler.obtainMessage(MESSAGE_POST_RESULT,
                        new AsyncTaskExResult<Result>(AsyncTaskExAggressive.this, result));
                message.sendToTarget();
            }
        };
    }
    
    public AsyncTaskExAggressive() {
        mWorker = new WorkerRunnable<Params, Result>() {
            public Result call() throws Exception {
     ...
     */
    public final AsyncTaskExAggressive<Params, Progress, Result> execute(Params... params) {
        ...
        aggressiveExecutor.execute(mFuture);
    ...
    private static class AsyncTaskExResult<Data> {
        final AsyncTaskExAggressive mTask;
        final Data[] mData;

        AsyncTaskExResult(AsyncTaskExAggressive task, Data... data) {
            mTask = task;
            mData = data;
        }
    }
